...
function lastPossessionsFromLog(opts: { log: PossLog[]; aBox: BoxPlayerLine[]; bBox: BoxPlayerLine[]; rand: () => number }) {
  ...
  return last.map((x) => {
    ...
    const funny = {
      made2: [
        (p: string) => `${p} gets to the rim and converts for 2`,
        (p: string) => `${p} scores 2 through contact that may or may not have existed`,
        (p: string) => `${p} finishes for 2 and jogs back like it was always inevitable`,
        (p: string) => `${p} hits a tough 2. The defense files a complaint`,
      ],
      miss2: [
        (p: string) => `${p} misses a 2. The rim remains undefeated`,
        (p: string) => `${p} comes up empty at the rim and immediately requests a review`,
        (p: string) => `${p} misses a short 2 and stares at the ball like it betrayed them`,
        (p: string) => `${p} can't get it to fall. So it goes`,
      ],
      made3: [
        (p: string) => `${p} knocks down a 3 with excellent timing and questionable confidence`,
        (p: string) => `${p} buries a 3. The lead feels different now`,
        (p: string) => `${p} hits a 3 and the defense rethinks its life choices`,
        (p: string) => `${p} drains a 3. Somebody somewhere says "that's a bad shot"`,
      ],
      miss3: [
        (p: string) => `${p} misses a 3. Ambitious`,
        (p: string) => `${p} can't connect from deep. The shot selection committee meets`,
        (p: string) => `${p} misses a 3 and the rebound takes a scenic route`,
        (p: string) => `${p} fires a 3 that does not receive tenure`,
      ],
      turnover: [
        (p: string) => `${p} turns it over. Unforced, but memorable`,
        (p: string) => `${p} coughs it up and politely invites the other team to score`,
        (p: string) => `${p} commits a turnover and looks around for a co-signer`,
        (p: string) => `${p} loses the ball. The possession ends early for efficiency`,
      ],
      orebPutbackMake: [
        (p: string) => `${p} snags the offensive board and sticks the putback for 2`,
        (p: string) => `${p} cleans it up for a putback. Second chances matter`,
      ],
      orebPutbackMiss: [
        (p: string) => `${p} gets the offensive rebound, then misses the putback. Twice is a choice`,
        (p: string) => `${p} grabs the OREB and can't finish the second chance`,
      ],
      orebKickoutMake: [
        (r: string, s: string) => `${r} wins an extra possession, kicks to ${s}. 3 is good`,
        (r: string, s: string) => `${r} keeps it alive. ${s} hits the kickout 3`,
      ],
      orebKickoutMiss: [
        (r: string, s: string) => `${r} keeps it alive, ${s} misses the kickout 3`,
        (r: string, s: string) => `${r} earns a second chance. ${s} can't cash it in from deep`,
      ],
    };
