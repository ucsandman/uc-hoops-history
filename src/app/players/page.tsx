...
import { coachForYear, eraForCoach, playerSeasons } from "@/lib/ucData";
...
export default function PlayersPage() {
  ...
  }, [q, minG, minMPG, minPPG, minRPG, minAPG]);

  const [sortBy, setSortBy] = useState<
    "name" | "ppg" | "rpg" | "apg" | "mpg" | "games"
  >("name");
      ...
      <header className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-300 max-w-2xl">
            Filter by season stats. This is per season, so a player can show up multiple times.
          </p>
        </div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-2">
                {c.rows.slice(0, 4).map((r) => {
                  ...
                  return (
                    <div
                      key={`${c.name}-${r.year}`}
                      className="rounded-lg border border-white/10 bg-black/20 p-3 text-xs text-zinc-200"
                    >
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-gradient-to-br from-black/30 via-white/5 to-black/20 p-5">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-lg font-black tracking-tight">Leaderboards</h2>
        <div className="text-xs text-zinc-500">Filtered by your sliders above</div>
      </div>

      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
        {blocks.map((b) => {
          const list = top(b.key);
          return (
            <div key={b.key} className="rounded-xl border border-white/10 bg-black/20 p-4">
              <div className="text-sm font-semibold text-zinc-100">{b.label}</div>
              <div className="mt-3 space-y-2">
                {list.map((r, idx) => (
                  <div
                    key={`${b.key}-${r.player}-${r.year}`}
                    className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs"
                  >
                    <div className="min-w-0">
                      <div className="truncate font-semibold text-zinc-100">
                        {idx + 1}. {r.player}
                      </div>
                      <div className="text-zinc-400">{r.year}–{String(r.year + 1).slice(-2)}</div>
                    </div>
                    <div className="shrink-0 text-zinc-200">
                      <span className="font-black">{r[b.key] ?? "—"}</span> <span className="text-zinc-500">{b.unit}</span>
                    </div>
                  </div>
                ))}
              </div>
